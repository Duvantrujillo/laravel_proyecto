                    ...
                    <table class="table table-sm table-borderless align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr class="hover-row">
                                ...
                                <td class="text-end">{{ number_format($monitoring->average_weight, 1) }}</td>
                                <td class="text-end">{{ number_format($monitoring->fish_balance) }}</td>
                                <td class="text-end">{{ $monitoring->biomass_percentage }}</td>
                                <td class="text-end">{{ number_format($monitoring->biomass, 1) }}</td>
                                <td class="text-end">{{ number_format($monitoring->daily_feed, 1) }}</td>
                                <td class="text-end">{{ $monitoring->ration_number }}</td>
                                <td class="text-end">{{ $monitoring->ration }}</td>
                                <td class="text-end text-success">{{ number_format($monitoring->weight_gain, 1) }}</td>
                                <td class="text-end">
                                    <span class="mortality-indicator {{ $monitoring->cumulative_mortality > 5 ? 'high' : 'low' }}">
                                        {{ number_format($monitoring->cumulative_mortality, 1) }}
                                    </span>
                                </td>
